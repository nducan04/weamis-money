...
<body class="text-slate-800 dark:text-slate-100 min-h-screen flex flex-col transition-colors duration-200">
    ...
    <!-- Top Navigation Header (Bright & Modern Theme) -->
    <header class="bg-white/90 dark:bg-slate-900/90 backdrop-blur-md sticky top-0 z-30 shadow-sm border-b border-slate-200/80 dark:border-slate-800 safe-top transition-colors">
        <div class="max-w-7xl mx-auto px-3 sm:px-6 py-2.5 sm:py-3.5 flex items-center justify-between">
            <div class="flex items-center space-x-2.5 sm:space-x-3 min-w-0">
                <a href="{{ route('dashboard') }}" class="flex items-center space-x-2.5 sm:space-x-3 min-w-0">
                    <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-emerald-500 text-slate-950 font-black text-base sm:text-xl flex items-center justify-center shadow-lg shadow-emerald-500/25 flex-shrink-0">
                        💲
                    </div>
                    <div class="min-w-0">
                        <h1 class="font-black text-base sm:text-xl tracking-tight leading-none text-slate-900 dark:text-white truncate">weamis-money</h1>
                        <p class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 font-semibold hidden xs:block">Quản lý thu chi & Quỹ team</p>
                    </div>
                </a>
            </div>
            
            <div class="flex items-center space-x-2 sm:space-x-3 flex-shrink-0">
                <span class="hidden lg:inline-flex items-center px-3 py-1 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/50 text-emerald-700 dark:text-emerald-300 text-xs font-bold rounded-full">
                    🏢 Quỹ: Trả nợ thuê Ltd
                </span>

                <!-- Dark Mode Toggle -->
                <button @click="darkMode = !darkMode" class="p-2 sm:p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-700 dark:text-slate-200 flex items-center space-x-1.5 transition">
                ...
                @auth
                    ...
                    <div class="flex items-center space-x-2 pl-2 border-l border-slate-200 dark:border-slate-800">
                        <div class="flex items-center space-x-2 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/80 px-2.5 py-1 rounded-xl">
                            ...
                            <div class="hidden sm:block text-left">
                                <p class="text-xs font-extrabold text-slate-900 dark:text-white leading-none">{{ Auth::user()->name }}</p>
                        ...
                        <form action="{{ route('logout') }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" title="Đăng xuất" class="p-2 bg-slate-100 dark:bg-slate-800 hover:bg-rose-600 hover:text-white border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 rounded-xl text-xs font-bold transition flex items-center justify-center">
                ...
                @else
                    <div class="flex items-center space-x-2">
                        <a href="{{ route('login') }}" class="px-3.5 py-2 bg-emerald-600 text-white rounded-xl text-xs font-bold hover:bg-emerald-700 transition shadow-sm">Đăng Nhập</a>
                    </div>
                @endauth
            </div>
        </div>
    ...
    <!-- PrebuiltUI Inspired Footer -->
    <footer class="bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 pt-10 px-4 sm:px-6 md:px-12 lg:px-20 xl:px-28 mt-12 safe-bottom transition-colors">
        <div class="max-w-7xl mx-auto">
            <div class="flex flex-wrap justify-between gap-10 md:gap-6">
                <!-- Column 1: Brand & Info -->
                <div class="max-w-xs">
                    <div class="flex items-center space-x-2.5 mb-4">
                        <div class="w-8 h-8 rounded-xl bg-emerald-500 text-slate-950 font-black text-lg flex items-center justify-center shadow-md">
                            💲
                        </div>
                    ...
                    <div class="flex items-center gap-3.5 mt-5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                        <!-- Instagram -->
                ...
                <!-- Column 2: COMPANY -->
                <div>
                    <p class="text-sm font-black uppercase tracking-wider text-slate-900 dark:text-white">CÔNG TY</p>
                    <ul class="mt-4 flex flex-col gap-2.5 text-sm font-semibold">
                ...
                <!-- Column 3: SUPPORT -->
                <div>
                    <p class="text-sm font-black uppercase tracking-wider text-slate-900 dark:text-white">HỖ TRỢ</p>
                    <ul class="mt-4 flex flex-col gap-2.5 text-sm font-semibold">
                ...
                <div class="max-w-xs">
                    <p class="text-sm font-black uppercase tracking-wider text-slate-900 dark:text-white">ĐĂNG KÝ NHẬN TIN</p>
                    <p class="mt-4 text-sm leading-relaxed text-slate-600 dark:text-slate-400 font-medium">
                        Đăng ký nhận bản tin cập nhật biến động quỹ và báo cáo thu chi hàng tháng.
                    </p>
                    <div class="flex items-center mt-4">
                        <input type="text" class="bg-slate-50 dark:bg-slate-800 rounded-l-xl border border-slate-300 dark:border-slate-700 h-10 px-3.5 text-sm font-medium focus:ring-2 focus:ring-emerald-500 outline-none text-slate-900 dark:text-white w-full" placeholder="Email của bạn" />
                        <button class="flex items-center justify-center bg-slate-900 hover:bg-emerald-600 dark:bg-slate-700 dark:hover:bg-emerald-600 text-white h-10 w-10 aspect-square rounded-r-xl transition flex-shrink-0">
                            <!-- Arrow icon -->
                            <svg class="w-4 h-4 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 12H5m14 0-4 4m4-4-4-4" /></svg>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row gap-3 items-center justify-between py-6 text-xs font-semibold">
